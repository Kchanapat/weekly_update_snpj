...
- Benchmark reference
  ...
  - **BigEarthNet-MM: A Large Scale Multi-Modal Multi-Label Benchmark Archive for Remote Sensing Image Classification and Retrieval** : ใช้การ retrieval เพื่อดึงภาพที่มี labels เหมือนกับภาพ query (multi-label retrieval) , วัด performance ด้วย metrics เช่น precision, recall, F2‑measure และ Hamming loss(ใช้วัดความผิดพลาดในการทำนาย โดยดูว่า ตำแหน่งของ label ที่เราทำนายผิด มีสัดส่วนเท่าไรเมื่อเทียบกับจำนวน label ทั้งหมด) สำหรับแต่ละ label/class
